...
function UpdateProfileForm({
  ...
  const onSubmit = async (displayName: string, photoFile: Maybe<File>) => {
    ...
    const promise = updateProfileMutation.mutateAsync(info).then(() => {
...
function RemovePhoneNumberButton({
  ...
  const onUnlinkPhoneNumber = useCallback(() => {
    const promise = unlinkProfileNumberMutation.mutateAsync().then(() => {
    ...
    <>
      ...
      >
        <div className={'flex flex-col space-y-3'}>
          ...
          <Button
            block
            loading={unlinkProfileNumberMutation.isLoading}
            color={'danger'}
